"""Reference subproblem solver (NumPy) and dual recovery helpers for LP subproblems."""
import time
...
def my_subproblem_solver(y_cur, mproblem_dict):
    ...
    for i in range(N):
        for l in range(L):
            ...
            index = i * L + l
            if not y_cur[index]:
                T_scaled[i, :, l] = float("inf")

    T_min_Rx = np.min(T_scaled, axis=0)
...
def compute_dual_solution(A, b, c, x_star, tol=1e-6):
    ...
    A_active = A[active_constraints]

    A_eq = A_active.T
    A_eq = A_eq[not_zero_x_condition]
    ...
    if np.any(y_dual < -tol):
        print("Warning: dual multiplier has negative entries.")
...
def solving_dual_solution(A, b, c, x_star, tol=1e-6):
    ...
    if result_1.success:
        ...
        raise ValueError("Dual recovery failed")
